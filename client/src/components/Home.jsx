      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
        {filteredProducts.map((product) => (
          ...
          >
            ...
            <div className="p-4 sm:p-5 pt-4 mt-auto border-t border-gray-100 flex items-center justify-between gap-2">
              <span className="text-lg sm:text-xl font-extrabold text-gray-900">
                ₹{product.price.toFixed(2)}
              </span>
              <button
                onClick={() => addToCart(product)}
                className="bg-gray-900 hover:bg-indigo-600 text-white px-3 py-2 rounded-lg font-bold text-sm sm:text-base transition-colors whitespace-nowrap"
              >
                Add to Cart
              </button>
            </div>
